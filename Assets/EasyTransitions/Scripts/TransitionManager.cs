...
namespace EasyTransition
{
    ...
    public class TransitionManager : MonoBehaviour
    {
        ...
        /// <param name="startDelay">The delay before the transition starts.</param>
        public void Transition(string sceneName, TransitionSettings transition, float startDelay, LoadSceneMode loadSceneMode = LoadSceneMode.Single)
        {
            if (transition == null || runningTransition)
            {
            ...
            runningTransition = true;
            StartCoroutine(Timer(sceneName, startDelay, transition, loadSceneMode));
        }

        /// <summary>
        /// Loads the new Scene with a transition.
        /// </summary>
        /// <param name="sceneNameLoad">The name of the scene you want to load.</param>
        /// <param name="transition">The settings of the transition you want to use to load you new scene.</param>
        /// <param name="startDelay">The delay before the transition starts.</param>
        public void TransitionChangeScene(string sceneNameLoad, TransitionSettings transition, float startDelay, LoadSceneMode loadSceneMode = LoadSceneMode.Single)
        {
            if (transition == null || runningTransition)
            {
                Debug.LogError("You have to assing a transition.");
            ...
            runningTransition = true;
            StartCoroutine(TimerChangeScene(sceneNameLoad, startDelay, transition, loadSceneMode));
        }
        ...
        IEnumerator Timer(string sceneName, float startDelay, TransitionSettings transitionSettings, LoadSceneMode loadSceneMode = LoadSceneMode.Single)
        {
        ...
        IEnumerator TimerChangeScene(string sceneNameLoad, float startDelay, TransitionSettings transitionSettings, LoadSceneMode loadSceneMode = LoadSceneMode.Single)
        {
            yield return new WaitForSecondsRealtime(startDelay);
            ...
            AsyncOperation asyncOp = SceneManager.LoadSceneAsync(sceneNameLoad, loadSceneMode);
