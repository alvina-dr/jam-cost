...
namespace BennyKok.RuntimeDebug.Components.UI
{
    ...
    public class DebugUIHandler : MonoBehaviour
    {
        ...
        public ListItem RootItem => rootItem;
        ...
        private void SearchItems(string query)
        {
            ...
            foreach (var item in cachedTriggers)
            {
                ...
                string name = item.actionTrigger.name.ToLower();
        ...
        public void OnAddAction(BaseDebugAction action)
        {
            ...
            List<string> groupDecomposition = action.group.Split('/').ToList();
            ListItem parentGroup = FindGroupInListRecursive(rootItem.children, groupDecomposition);

            if (parentGroup != null) // group already exists
            {
                var newItem = new ListItem(action);
                parentGroup.children.Add(newItem);
                newItem.parent = parentGroup;
                if (parentGroup.uiList != null) parentGroup.uiList.RowCount = parentGroup.children.Count;
                return;
            }
            else // group doesn't exist
            {
                ListItem closestParent = rootItem;
                for (int i = 0; i < groupDecomposition.Count; i++)
                {
                    int index = i;

                    ListItem group = FindGroupInList(closestParent.children, groupDecomposition[index]);
                    if (group != null)
                    {
                        closestParent = group; 
                    }
                    ...
                    {
                        var newGroup = CreateGroup(groupDecomposition[index]);
                        newGroup.parent = closestParent;
                        //Make sure the group item is added after the current last group item
                        var lastGroupIndex = closestParent.children.FindLastIndex(x => !string.IsNullOrEmpty(x.groupName)) + 1;
                        closestParent.children.Insert(lastGroupIndex, newGroup);

                        closestParent = newGroup;
                    }
                }

                var newActionItem = new ListItem(action);
                closestParent.children.Add(newActionItem);
                newActionItem.parent = closestParent;
            }

            //If item has no group
            if (!hasGroup)
            {
        ...
        public ListItem FindGroupInList(List<ListItem> list, string groupName)
        {
        ...
        public ListItem FindGroupInListRecursive(List<ListItem> list, List<string> groupDecompositionCopy)
        {
            List<string> groupDecomposition = new List<string>(groupDecompositionCopy);
            
            for (int i = 0; i < list.Count;i++)
            {
                //Skipping, this is a root trigger
                if (!list[i].IsGroup) continue;

                if (list[i].groupName == groupDecomposition[0])
                {
                    //if (_item.uiList != null) _item.uiList.RowCount = _item.children.Count;
                    if (groupDecomposition.Count > 1)
                    {
                        groupDecomposition.RemoveAt(0);
                        return FindGroupInListRecursive(list, groupDecomposition);
                    }
                    else
                    {
                        return list[i];
        ...
        public ListItem CreateGroup(string name)
        {
            var item = new ListItem();
